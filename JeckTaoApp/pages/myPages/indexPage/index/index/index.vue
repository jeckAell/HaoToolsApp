<template>
	<view class="uni-container">
		...
		<view class="index-content">
			<view v-if="appToolDatas && appToolDatas.length > 0">
				<view 
				 v-for="item in appToolDatas"
				 :key="item.name"
				 @click="junpPage(item.url)"
				 class="uni-flex uni-row one-tool">
			...
			<!-- #ifdef H5 -->
			<view v-if="h5ToolDatas && h5ToolDatas.length > 0">
				<view 
				 v-for="item in h5ToolDatas"
				 :key="item.name"
				 @click="junpPage(item.url)"
				 class="uni-flex uni-row one-tool">
				 <view class="flex-item">
...
<script>
	...
	import { http, debounce } from '@/untils/request.js'
	export default {
		components: {
		    uLink
		},
			...
			http('http://localhost:8980/jecktao/tools/queryAll').then(res => {
				...
				res.data.forEach(item => {
					if (item.platforms !== 'h5') {
						this.appToolDatas.push(item)
					} else {
						this.h5ToolDatas.push(item)
					}
				})
			})
		},
		...
		methods: {
			changeInput(event) {
				...
				this.timer = setTimeout(() => {
					http('http://localhost:8980/jecktao/tools/queryListByName', 'POST', this.searchInput ? this.searchInput : ' ').then( res => {
						this.appToolDatas = []
						this.h5ToolDatas = []
						res.data.forEach(item => {
							if (item.platforms !== 'h5') {
...
.uni-container {
	.uni-header-logo {
		margin-top: 0;
		height: 140rpx;
		padding: 80rpx 40rpx 110rpx 40rpx;
	...
	.index-content {
		...
		.one-tool {
			...
			&:active {
				box-shadow: 0px 0px 8px 1px #4f4f4f;
			}
